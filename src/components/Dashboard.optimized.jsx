/**
 * Optimized Dashboard component with accessibility and performance improvements
 * ARIA labels, semantic HTML, and memoized calculations
 */
...
const Dashboard = memo(({ carbonSaved, waterSaved, wasteDiverted, actions, onToggleAction }) => {
  ...
  const carbonMetrics = calculateCarbonMetrics(carbonSaved);
  const waterMetrics = calculateWaterMetrics(waterSaved);
  const wasteMetrics = calculateWasteMetrics(wasteDiverted);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      
      {/* 1. Core Analogies Top Row */}
      <section className="grid-3" role="region" aria-label="Summary metrics">
        <div 
          className="glass-card emerald-glow" 
          style={{ padding: '20px', display: 'flex', gap: '16px', alignItems: 'center' }}
          role="complementary"
          aria-label={createMetricLabel('Carbon Analogy', carbonMetrics.treeYears, 'tree-years')}
        >
          <div style={{ fontSize: '32px', background: 'rgba(16, 185, 129, 0.1)', padding: '12px', borderRadius: '12px', border: '1px solid rgba(16, 185, 129, 0.2)', role: 'img', 'aria-label': 'Trees icon' }}>🌲</div>
          <div>
            <h4 style={{ fontSize: '13px', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Carbon Analogy</h4>
            <div style={{ fontSize: '20px', fontWeight: '800', margin: '4px 0 2px' }}>{carbonMetrics.treeYears} Tree-Years</div>
            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Equivalent mature oak trees absorbing CO2 for a year, or {carbonMetrics.kmDriven} km petrol driving avoided.</p>
          </div>
        </div>
        ...
        <div 
          ...
          style={{ padding: '20px', display: 'flex', gap: '16px', alignItems: 'center' }}
          role="complementary"
          aria-label={createMetricLabel('Water Preserved', waterSaved, 'liters')}
        >
          <div style={{ fontSize: '32px', background: 'rgba(6, 182, 212, 0.1)', padding: '12px', borderRadius: '12px', border: '1px solid rgba(6, 182, 212, 0.2)', role: 'img', 'aria-label': 'Water droplet icon' }}>💧</div>
          <div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Equal to {waterMetrics.showersSaved} standard 8-minute showers saved through lifestyle dietary choices.</p>
          </div>
        </div>
        ...
        <div 
          ...
          style={{ padding: '20px', display: 'flex', gap: '16px', alignItems: 'center' }}
          role="complementary"
          aria-label={createMetricLabel('Waste Diverted', wasteDiverted, 'kg')}
        >
          <div style={{ fontSize: '32px', background: 'rgba(255, 255, 255, 0.05)', padding: '12px', borderRadius: '12px', border: '1px solid var(--border-light)', role: 'img', 'aria-label': 'Recycle icon' }}>♻️</div>
          <div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Equivalent to keeping approximately {wasteMetrics.bottlesDiverted} single-use plastic bottles out of landfills.</p>
          </div>
        </div>
      </section>
      ...
      <section className="grid-2">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Daily Action Cards */}
          <div className="glass-card" style={{ padding: '20px' }} role="region" aria-label="Action cards">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: actions.solar ? 'rgba(16, 185, 129, 0.05)' : 'rgba(255, 255, 255, 0.02)', border: actions.solar ? '1px solid var(--accent-emerald)' : '1px solid var(--border-light)', borderRadius: '12px', transition: 'all 0.3s ease' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  <span style={{ fontSize: '20px', role: 'img', 'aria-label': 'Solar panels icon' }}>☀️</span>
                  <div>
                ...
                <button 
                  ...
                  aria-label={createActionLabel('Community Solar Link', actions.solar, '+50 CC')}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: actions.bike ? 'rgba(16, 185, 129, 0.05)' : 'rgba(255, 255, 255, 0.02)', border: actions.bike ? '1px solid var(--accent-emerald)' : '1px solid var(--border-light)', borderRadius: '12px', transition: 'all 0.3s ease' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  <span style={{ fontSize: '20px', role: 'img', 'aria-label': 'Bicycle icon' }}>🚲</span>
                  <div>
                ...
                <button 
                  ...
                  className="btn-primary"
                  aria-label={createActionLabel('Cycle Commute Challenge', actions.bike, '+25 CC')}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: actions.thermostat ? 'rgba(16, 185, 129, 0.05)' : 'rgba(255, 255, 255, 0.02)', border: actions.thermostat ? '1px solid var(--accent-emerald)' : '1px solid var(--border-light)', borderRadius: '12px', transition: 'all 0.3s ease' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  <span style={{ fontSize: '20px', role: 'img', 'aria-label': 'Thermometer icon' }}>🌡️</span>
                  <div>
                ...
                <button 
                  ...
                  className="btn-primary"
                  aria-label={createActionLabel('Smart Thermostat Tuning', actions.thermostat, '+10 CC')}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: actions.meat ? 'rgba(16, 185, 129, 0.05)' : 'rgba(255, 255, 255, 0.02)', border: actions.meat ? '1px solid var(--accent-emerald)' : '1px solid var(--border-light)', borderRadius: '12px', transition: 'all 0.3s ease' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  <span style={{ fontSize: '20px', role: 'img', 'aria-label': 'Vegetable salad icon' }}>🥗</span>
                  <div>
                ...
                <button 
                  ...
                  className="btn-primary"
                  aria-label={createActionLabel('Meat-Free Day Swap', actions.meat, '+20 CC')}
          ...
          <div className="glass-card" style={{ padding: '20px' }} role="region" aria-label="Integration status">
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,0.03)', padding: '8px 12px', borderRadius: '10px', border: '1px solid var(--border-light)', fontSize: '12px' }}>
                <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#10b981', display: 'inline-block' }} className="animate-pulse-glow" aria-label="Connected status"></span>
      ...
      <section className="glass-card" style={{ padding: '20px' }} role="region" aria-label="Telemetry logs">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-light)', borderRadius: '10px', fontSize: '13px' }} role="log">
            <div style={{ display: 'flex', gap: '12px' }}>
              <span style={{ color: 'var(--accent-emerald)', 'aria-label': 'Energy icon' }}>⚡</span>
              <span><b>UtilityAPI</b>: Home battery storage charging delayed to clean grid hours.</span>
            </div>
            <span style={{ color: 'var(--text-muted)', 'aria-label': 'Timestamp' }}>10m ago</span>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-light)', borderRadius: '10px', fontSize: '13px' }} role="log">
            <div style={{ display: 'flex', gap: '12px' }}>
              <span style={{ color: 'var(--accent-cyan)', 'aria-label': 'Walking icon' }}>🚶</span>
              <span><b>CoreMotion</b>: Auto-detected walk (1.2 km). Logged under commuter savings.</span>
            </div>
            <span style={{ color: 'var(--text-muted)', 'aria-label': 'Timestamp' }}>1h ago</span>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-light)', borderRadius: '10px', fontSize: '13px' }} role="log">
            <div style={{ display: 'flex', gap: '12px' }}>
              <span style={{ color: 'var(--accent-orange)', 'aria-label': 'Credit card icon' }}>💳</span>
              <span><b>Plaid API</b>: Transaction category <i>Supermarket</i> scanned. Food LCA catalog loaded.</span>
            </div>
            <span style={{ color: 'var(--text-muted)', 'aria-label': 'Timestamp' }}>3h ago</span>
          </div>
        </div>
      </section>
    </div>
  ...
  return (
    prevProps.carbonSaved === nextProps.carbonSaved &&
    prevProps.waterSaved === nextProps.waterSaved &&
    prevProps.wasteDiverted === nextProps.wasteDiverted &&
